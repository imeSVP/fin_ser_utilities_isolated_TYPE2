        ...
        for data in final:
            ...
            for _ in links:
                ...
                if "miaSerena GAS" in data[1]:
                    ...
                    elif "mia-protetta" in data[2]:
                        prdts.append([data[0],'miaProtetta LUCE',data[2],_.a['href']])
                    else:
                    ...
                    if "mia-convenienza" in data[2]:
                        ...
                        prdts.append([data[0],'miaProtetta GAS',data[2],_.a['href']])
                    else:
            ...
            try:
                ...
                if os.path.exists(dest_path):
                    if os.path.isfile(dest_path):
                        try:
                            os.remove(dest_path)
                            print(f"[INFO] File esistente rimosso: {dest_path}")
                        except PermissionError:
                            # 尝试修改权限后删除
                            try:
                                os.chmod(dest_path, 0o777)
                                os.remove(dest_path)
                                print(f"[INFO] File esistente rimosso (dopo chmod): {dest_path}")
                            except PermissionError as e:
                                print(f"[ERRORE] Impossibile rimuovere file occupato: {dest_path}")
                                print(f"[ERRORE] Chiudere eventuali programmi che lo utilizzano (PDF reader, etc.)")
                                # 删除临时文件
                                if os.path.exists('file.pdf'):
                                    os.remove('file.pdf')
                                continue
                    else:
                        print(f"[ERRORE] Il percorso {dest_path} è una directory, non un file")
                        if os.path.exists('file.pdf'):
                            os.remove('file.pdf')
                        continue
                ...
                print(f"[SUCCESSO] File salvato: {dest_path}")

                # Imposta permessi (facoltativo: dipende dal sistema)
                try:
                    os.chown(dest_path, 1003, 1004)
                except Exception as e:
                    print(f"[WARNING] Impossibile impostare permessi: {e}")
                ...
                print(f"[EXCEPTION] Errore imprevisto: {type(e).__name__}: {e}")
                # 清理临时文件
                if os.path.exists('file.pdf'):
                    try:
                        os.remove('file.pdf')
